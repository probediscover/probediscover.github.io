...
function locationSampleSoldChart(){
  ...
  for (let i = 0; i < campaign['locations'].length; i++){
    let locationName = campaign['locations'][i]['location'].trim();
    ...
    if (locationIndex === -1){
      ...
      locationNotSold[locationIndex] = 0;
      locationSampled[locationIndex] = 0;
    }
  }
  for (let i = 0; i < surveys.length; i++){
    let locationName = surveys[i]['location'].trim();
    let locationIndex = locationLabels.indexOf(locationName);
...
function loadDayDateForLocation(location){
  ...
  for (let i = 0; i < surveys.length; i++){
    let locationName = surveys[i]['location'].trim();
    if (location !== 'All' && locationName !== location.trim())
      continue;
...
function loadConversionForAllLocations(){
  ...
  for (let i = 0; i < locationSold.length; i++){
    if (locationNotSold[i] === 0)
      locationNotSold[i] = 1;
    conversion.push(+((locationSold[i] / locationNotSold[i]).toFixed(2)));
  }
